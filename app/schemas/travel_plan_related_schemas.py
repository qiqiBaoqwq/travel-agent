...
class AppResponse(BaseModel, Generic[T]):
    ...
    统一Web请求返回结果类
    """
    code: str = Field(default="", description="返回的状态码")
    message: str = Field(default="", description="返回的消息")
    ...
    @staticmethod
    def success(data: Optional[Any] = None, message: str = "处理成功",
                code: str = "200",
                result_code: Optional["ResultCode"] = None) -> "AppResponse":
        """成功的响应"""
        if result_code:
            return AppResponse(code=result_code.code, message=result_code.message,
                             data=data)
        return AppResponse(code=code, message=message, data=data)

    @staticmethod
    def fail(data: Optional[Any] = None, message: str = "系统内部错误",
                code: str = "500",
                result_code: Optional["ResultCode"] = None) -> "AppResponse":
        """失败的响应"""
        if result_code:
